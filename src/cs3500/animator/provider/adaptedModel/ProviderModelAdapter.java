 ...
 */
public class ProviderModelAdapter extends ReadOnlyProviderModelAdapter
    implements ExCELenceAnimatorModel {
  /**
   * Adapter constructor to adapt AnimationModel to provider's ExCELenceAnimatorModel.
   * @param model the model for the adapter to use
   */
  ...
  public void declareShape(String name, Shape2D.ShapeType type) {
    ...
    model.addShape(s);
    model.addKeyFrame( name, 0, (int)s.getPosition().getXCoord(),
        (int)s.getPosition().getYCoord(), (int)s.getSize().getW(), (int)s.getSize().getH(),
